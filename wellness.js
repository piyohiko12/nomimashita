...
export function validateWellness(value) {
  ...
  return {mood:value.mood,symptoms:SYMPTOMS.filter(s=>value.symptoms.includes(s.id)).map(s=>s.id),note:value.note.trim()};
}
...
export function wellnessBanner(record, date) {
  return '<button class="wellness-banner" data-wellness-edit="'+escapeHtml(date)+'">'
    ...
    +'</strong><small>'+(record?'記録済み · タップして確認・編集':'顔と症状を選んで、今日の調子を残す')+'</small></span><span class="chevron" aria-hidden="true">›</span></button>';
}
