...
import { faceIcon, moodLabel, wellnessBanner, wellnessDetails, wellnessEditor } from './wellness.js';
    ...
    const themes = {simple:'シンプル',soft:'ふんわり可愛い',character:'キャラクター'};
    const themeNotes = {simple:'白とグリーンのすっきりしたデザイン',soft:'淡いピンクとハートのやさしいデザイン',character:'キャラクターとミント色の楽しいデザイン'};
    ...
    function home(){const reminders=due();return '<div class="day-heading"><p>'+today.replaceAll('-','/')+'</p><span>お薬のチェック</span></div>'+(reminders.length?'<div class="notice" role="status">設定した時刻を過ぎています<br>'+reminders.map(esc).join('<br>')+'<br><small>実際に飲んだことを確かめてチェックしてください。</small></div>':'')+medicineCards(today)+wellnessBanner(state.wellness[today],today)+'<p class="day-footnote">毎朝5:00に新しい日へ · 前日の記録は残ります</p>';}
    function calendar(){
    ...
    function help(){return back()
      ...
      +'<section class="section"><h2>体調を残す</h2><p>ホームの薬チェックの下から、顔・症状・メモを記録できます。顔を1つ選ぶだけでも保存できます。カレンダーの日付を選ぶと、内容の確認・編集ができます。</p><small>1日1つの体調記録です。薬と同じく日本時間の朝5:00が区切りで、0:00〜4:59は前日分になります。体調の記録は自己評価であり、診断を行うものではありません。</small></section>'
    ...
    function render(){if(!state)return;const theme=themes[state.settings.theme]?state.settings.theme:'simple';document.body.dataset.theme=theme;const title=draft?(editMode==='wellness'?'体調を記録':editMode==='medicine'?'薬の登録':'お知らせ時刻'):tab==='home'?'今日の記録':tab==='calendar'?'カレンダー':({medicines:'薬の登録',reminders:'アラーム・通知',reset:'チェックの更新時間',themes:'テーマ',help:'使い方'}[page]||'設定');$('heading').innerHTML='<div class="heading-copy"><div class="brand-lockup"><img class="brand-mark" src="./icon.svg" alt=""><span class="brand-name">ここち<span>ログ</span></span></div>'+(demo?'<span class="demo-badge">体験モード</span>':'')+'<h1>'+title+'</h1></div>'+decoration(theme);$('content').innerHTML=draft?(editMode==='wellness'?wellnessEditor(draft):editor()):tab==='home'?home():tab==='calendar'?calendar():page==='medicines'?listMedicines():page==='reminders'?listMedicines(true):page==='themes'?themeOptions():page==='reset'?resetPage():page==='help'?help():settings();$('nav').innerHTML=[['home','ホーム'],['calendar','カレンダー'],['settings','設定']].map(([id,label])=>'<button data-tab="'+id+'" aria-current="'+(tab===id?'page':'false')+'"><svg viewBox="0 0 24 24" aria-hidden="true">'+icons[id]+'</svg>'+label+'</button>').join('');$('saved').textContent=(demo?'体験用 · 記録は保存されません':'この端末に保存 · 日本時間');if(busy)lock(true);}
    ...
    function openWellness(date){if(!Core.validDay(date)||date<state.startedDay||date>today)return;const record=state.wellness[date];draft={day:date,mood:record?.mood||'',symptoms:clone(record?.symptoms||[]),note:record?.note||'',existing:!!record};editMode='wellness';render();window.scrollTo({top:0,behavior:'instant'});}
    function backup(){const blob=new Blob([JSON.stringify({app:'ここちログ',exportedAt:new Date().toISOString(),state},null,2)],{type:'application/json;charset=utf-8'});const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='cocochi-log-'+today+'.json';a.target='_blank';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),60000);}
    ...
    document.addEventListener('submit',e=>{
      ...
      if(e.target.id==='wellness-editor'){
        const data=new FormData(e.target),updated={...draft,mood:data.get('mood'),symptoms:data.getAll('symptoms'),note:data.get('note')};
