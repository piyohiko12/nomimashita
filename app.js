...
import { largeDate, verticalMedicines, calendarDoseDots } from './medicine-view.js';
    ...
    const themes = {coral:'やさしいコーラル',simple:'シンプル',soft:'ふんわり可愛い'};
    const themeNotes = {coral:'淡いコーラル色と、ゆったりした丸いカード',simple:'白とグリーンのすっきりしたデザイン',soft:'淡いピンクとハートのやさしいデザイン'};
    ...
    function decoration(theme){return theme==='soft'?'<span class="hearts" aria-hidden="true">✧<b>♡</b>✧</span>':'';}
    function back(){return '<button class="back" data-back="1">‹ 設定に戻る</button>';}
    function medicineCards(date){return Core.plans(state,date).map(m=>'<section class="section"><h2>'+esc(Core.title(m))+'</h2><div class="slots medicine-slots">'+m.slots.map((s,i)=>s.on?'<div class="slot"><span>'+labels[i]+'</span><button class="check" data-check="'+esc(m.id)+'" data-date="'+date+'" data-slot="'+i+'" aria-pressed="'+!!s.taken+'" aria-label="'+esc(m.name)+' '+labels[i]+' '+(s.taken?'飲んだ・チェックを外す':'飲んだと記録')+'"><span class="mark" aria-hidden="true">'+(s.taken?'☑':'□')+'</span><strong>'+(s.taken?'飲んだ':'未チェック')+'</strong><small>'+s.qty+'錠 · '+s.time+'</small></button></div>':'').join('')+'</div>'+(m.note?'<div class="note">'+esc(m.note)+'</div>':'')+'</section>').join('')||'<section class="section empty"><div class="symbol" aria-hidden="true">☑</div><p>この日の服薬予定はありません。</p>'+(date===today?'<button class="primary" data-add="1">＋ 薬を登録する</button>':'')+'</section>';}
    function due(){const now=new Date(Date.now()+offset+9*3600000),min=now.getUTCHours()*60+now.getUTCMinutes(),clock=min<300?min+1440:min;const result=[];for(const m of state.medicines){if(m.deletedDay&&m.deletedDay<=today)continue;const r=Core.revision(m,today);if(!r||!r.active)continue;const p=Core.plans(state,today).find(p=>p.id===m.id);r.slots.forEach((s,i)=>{const [h,n]=s.time.split(':').map(Number),raw=h*60+n,target=raw<300?raw+1440:raw;if(s.on&&s.remind&&clock>=target&&!p?.slots[i]?.taken)result.push(r.name+' '+labels[i]+'（'+s.qty+'錠）');});}return result;}
    ...
    function calendar(){
      ...
      for(let d=1;d<=count;d++){
        const date=month+'-'+String(d).padStart(2,'0'),disabled=date<state.startedDay||date>today,status=Core.summary(state,date),record=state.wellness[date],dots=disabled?{html:'',text:''}:calendarDoseDots(Core.plans(state,date));
        cells+='<button class="calendar-day" data-day="'+date+'" aria-label="'+date+' '+(disabled?'対象外':'体調 '+moodLabel(record?.mood)+'・'+dots.text)+'" aria-pressed="'+(date===selected)+'" data-state="'+(disabled?'empty':status)+'" '+(disabled?'disabled':'')+'><span class="calendar-number">'+d+'</span><span class="calendar-stamp" aria-hidden="true">'+(!disabled&&record?faceIcon(record.mood):'')+'</span>'+dots.html+'</button>';
      }
      return '<section class="section calendar-section"><div class="row"><button data-month="-1" aria-label="前の月" '+(month<=state.startedDay.slice(0,7)?'disabled':'')+'>‹</button><strong>'+y+'年'+m+'月</strong><button data-month="1" aria-label="次の月" '+(month>=today.slice(0,7)?'disabled':'')+'>›</button></div><div class="calendar">'+cells+'</div><div class="calendar-legend"><span>'+faceIcon('good')+'顔スタンプ：その日の体調</span><div class="dose-legend"><span><i class="dose-dot dose-dot-0 taken"></i>朝</span><span><i class="dose-dot dose-dot-1 taken"></i>昼</span><span><i class="dose-dot dose-dot-2 taken"></i>晩</span></div><small>色付き：服用済み　白い点：未チェック　半分：一部服用済み</small></div></section><div class="selected-date"><h2>'+selected.replaceAll('-','/')+' の記録</h2><p class="muted">日付を選ぶと、体調と服薬の記録が見られます。</p></div>'+wellnessDetails(state.wellness[selected],selected)+'<h3 class="medication-heading">お薬のチェック</h3>'+medicineCards(selected)+'<p class="muted">未チェックだけでは、実際に飲まなかったとは断定できません。記録開始前・未来の日は変更できません。</p>';
    }
    ...
    function listMedicines(reminders=false){const medicines=state.medicines.filter(m=>!m.deletedDay||m.deletedDay>today);return back()+(reminders?'<div class="notice">このWeb版は、画面を閉じている間のアラーム・プッシュ通知には対応していません。下で時刻を設定すると、ホームを開いている間にお知らせします。<br>音での通知には、iPhoneの「時計」アプリにもアラームを設定してください。</div>':'<p><button class="primary" data-add="1">＋ 薬を追加</button></p>')+medicines.map(m=>{const r=Core.revision(m,today);if(!r)return '';return '<button class="linkrow" '+(reminders?'data-reminder':'data-edit')+'="'+m.id+'"><strong>'+esc(r.name)+'</strong><small>'+(r.active?r.slots.map((s,i)=>s.on?labels[i]+' '+(reminders?s.time+'・'+(s.remind?'画面内お知らせON':'お知らせOFF'):s.qty+'錠'):'').filter(Boolean).join(' / '):'休止中')+'</small></button>';}).join('')+(!medicines.length?'<p class="muted">まず「薬の登録」で薬を追加してください。</p>':'');}
    ...
    function help(){return back()
      ...
      +'<section class="section"><h2>体調を残す</h2><p>ホームの薬チェックの下から、顔・症状・トイレ（排便）・メモを記録できます。トイレは「でた・でていない・未記録」から選べます。顔を1つ選ぶだけでも保存できます。カレンダーの日付を選ぶと、内容の確認・編集ができます。</p><small>1日1つの体調記録です。薬と同じく日本時間の朝5:00が区切りで、0:00〜4:59は前日分になります。入力していないトイレ状況は「でていない」ではなく「未記録」として残ります。体調の記録は自己評価であり、診断を行うものではありません。</small></section>'
    ...
    function editor(){const canDelete=editMode==='medicine'&&state.medicines.some(m=>m.id===draft.id&&(!m.deletedDay||m.deletedDay>today));return '<form id="editor"><section class="section">'+(editMode==='medicine'?'<label>薬の名前<input name="name" maxlength="80" required autocomplete="off" value="'+esc(draft.name)+'"></label>':'<h2>'+esc(draft.name)+'</h2><p class="muted">ここで設定するのは、画面を開いている間のお知らせです。音は鳴りません。</p>')+draft.slots.map((s,i)=>editMode==='medicine'?'<div class="formslot"><label><input type="checkbox" name="on'+i+'" '+(s.on?'checked':'')+'>'+labels[i]+'に飲む</label><label class="qty">'+labels[i]+'の錠数<input name="qty'+i+'" type="number" min="0.01" max="99" step="0.01" inputmode="decimal" value="'+s.qty+'" required></label></div>':s.on?'<div class="formslot"><h3>'+labels[i]+'</h3><label>飲む時刻<input type="time" name="time'+i+'" value="'+s.time+'" required></label><label><input type="checkbox" name="remind'+i+'" '+(s.remind?'checked':'')+'>画面内のお知らせを表示</label></div>':'').join('')+(editMode==='medicine'?'<label>メモ（任意）<textarea name="note" maxlength="300">'+esc(draft.note)+'</textarea></label><label><input type="checkbox" name="active" '+(draft.active?'checked':'')+'>この薬の予定を有効にする</label><small>変更は今日から反映します。過去の日の予定と、チェック済みの錠数は保持します。使わなくなった薬はチェックを外して休止できます。お知らせ時刻は「アラーム・通知」で設定します。</small>'+(canDelete?'<button class="delete-medicine" type="button" data-medicine-delete="'+esc(draft.id)+'">この薬を削除</button>':''):'<small>0:00〜4:59は前日分です。画面を閉じている間には通知されません。</small>')+'</section><div class="row savebar"><button type="button" data-cancel="1">キャンセル</button><button class="primary" type="submit">保存</button></div></form>';}
    function render(){
      if(!state)return;const theme=themes[state.settings.theme]?state.settings.theme:'coral',atHome=tab==='home'&&!draft;document.body.dataset.theme=theme;
    ...
    document.addEventListener('click',e=>{
      ...
      const go=()=>{
        ...
        else if(d.medicineDelete){const target=state.medicines.find(m=>m.id===d.medicineDelete),r=target&&Core.revision(target,today);if(!r){tell('削除する薬が見つかりません。再読み込みしてください。',true);return;}ask('「'+r.name+'」を薬の登録から削除しますか？\n今日以降の未チェック予定は表示されなくなります。過去と今日の服用済み記録は残ります。',()=>save({type:'medicine-delete',id:d.medicineDelete,day:today},()=>{draft=null;page='medicines';}),'薬を削除');return;}
